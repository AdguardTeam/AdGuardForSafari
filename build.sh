...
xcodebuild -workspace "$WORKSPACE" -scheme "$SCHEME" archive -configuration "$CONFIGURATION_NAME" -arch="arm64" -archivePath "$ARCHIVE_PATH" ONLY_ACTIVE_ARCH=YES VALID_ARCHS=arm64 -destination 'platform=OS X'
