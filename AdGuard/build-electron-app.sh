...
if [ ${CONFIGURATION} == "Release" ]; then
    ...
    electron-packager "${SRC}" "${PRODUCT_NAME}" --electron-version=6.1.8 --platform=${PLATFORM} --app-bundle-id="${AG_BUNDLEID}" \
    ...
    electron-osx-sign "${APP}" --platform=${PLATFORM} --type=distribution --hardened-runtime --version=6.1.8 --identity="${CODE_SIGN_IDENTITY}" --entitlements="${AG_APP_ENT}" || exit 1
    ...
    electron-packager "${SRC}" "${PRODUCT_NAME}" --electron-version=6.1.8 --platform=${PACKAGER_PLATFORM} --app-bundle-id="${AG_BUNDLEID}" \
    --arch=${ARCH} --app-version="${AG_VERSION}"  --build-version="${AG_BUILD}" --overwrite --out="${TARGET_TEMP_DIR}" \
    ${OPT} || exit 1
